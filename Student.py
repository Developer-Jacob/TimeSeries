...
def objective(trial, epochs, preprocessor, function_make_trainer, function_make_model):
    ...
    _, loss_valid = trainer.train(epochs, model, criterion, optimizer)
...
class Student:
    ...
    def best_learning_rate(self):
        return self.best_prams[Student.key_learning_rate]

    def best_hidden_size(self):
        return self.best_prams[Student.key_hidden_size]

    def best_dropout_rate(self):
        return self.best_prams[Student.key_dropout_rate]
    ...
    def study(self):
        ...
        study.optimize(lambda trial:
                       objective(
                        ...
                        self.epochs,
                        self.preprocessor,
                        self.function_make_trainer,
                        self.function_make_model
                       ),
                       n_trials=500
                       )
    ...
    def train_with_best_params(self):
        ...
        trained_model, loss_valid = trainer.train(self.epochs, lstm_model, criterion, optimizer)
