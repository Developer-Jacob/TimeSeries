...
def FFT_for_Period(x, k=2):
    """
    x: (B, T, C)  —  이 함수 안에서는 CPU에서 FFT → 결과를 다시 원래 디바이스로
    """
    # 1) CPU FFT
    xf_cpu = torch.fft.rfft(x.detach().cpu(), dim=1)        # (B, F, C) 복소

    # 2) **채널 평균을 반드시 포함**해 원래와 동일한 출력 형태로 맞춘다
    mag = xf_cpu.abs().mean(-1).to(x.device)                # (B, F) <-- ★

    # 3) 주기 탐색 (batch 축은 평균해도 무방)
    frequency_list = mag.mean(0)                            # (F,)
    frequency_list[0] = 0
    _, top_list = torch.topk(frequency_list, k)             # (k,)

    period  = x.shape[1] // top_list.cpu()                  # (k,)
    return period, mag[:, top_list]                         # (k,) , (B, k)
